...
impl KvStore {
    ...
    pub fn open(path: impl Into<PathBuf>) -> Result<KvStore> {
        ...
        for term in terms {
            ...
            load_file(term, &mut readers, &mut index)?;
        }
...
fn load_file(term: u64, readers: &mut HashMap<u64, File>, index: &mut BTreeMap<String, Pos>) -> Result<()> {
    ...
    while let Some(res) = stream.next() {
        ...
        match res? {
            Command::Set { key, .. } => index.insert(key, pos),
